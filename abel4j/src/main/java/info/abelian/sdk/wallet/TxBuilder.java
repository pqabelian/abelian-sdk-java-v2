...
public class TxBuilder extends AbelBase {
  ...
  public UnsignedRawTx build() throws AbelException {
    ...
    int[] signerAccountIDs = new int[txInDescs.size()];
    for (int i = 0; i < txInDescs.size(); i++) {
      TxInDesc txInDesc = txInDescs.get(i);
      ViewAccount ownedViewAccount = viewer.getOwnedViewAccount(txInDesc.coin);
      signerAccountIDs[i]=ownedViewAccount.id;
    }
  ...
  protected boolean verifyAndCompleteTxInDescs() {
    ...
    for (int i = 0; i < txInDescs.size(); i++) {
      ...
      if (!coinOnChain.txOutData.equals(coinGiven.txOutData)) {
